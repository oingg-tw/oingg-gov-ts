 ...
 * 那個參數化查詢的路徑上。
 *
 * **機制目前未知。** 曾經懷疑是 node-postgres pool 裡每條連線各自保留一塊讀取 buffer，
 * oingg-sitca-ts 2026-09-24 用 pool max 做了判別實驗，**否證**：同批量下 pool=10 與 pool=2 的
 * heap 平台都是 125MB（若是 per-connection，pool=2 該只有五分之一）。所以那塊保留是
 * per-process／per-client 的，不是每條連線一份。飽和為什麼由累積呼叫次數決定，目前沒有解釋。
 *
